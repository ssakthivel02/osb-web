...
export type TrainingRelationship = { relationship_id:string; from_id:string; to_id:string; relationship_type:'PREREQUISITE_OF'|'CROSS_LINK'; rationale:string; };

const ROOT = path.join(process.cwd(),'data','training-academy');
const EXPANSION_ROOT = path.join(ROOT,'expansion');

function expansionFiles(filename:string):string[]{
  if(!fs.existsSync(EXPANSION_ROOT)) return [];
  return fs.readdirSync(EXPANSION_ROOT,{withFileTypes:true})
    .filter(entry=>entry.isDirectory())
    .map(entry=>`expansion/${entry.name}/${filename}`)
...
function readJsonl(p:string):JsonRecord[]{
  return fs.readFileSync(path.join(ROOT,p),'utf8').split(/\r?\n/).map(x=>x.trim()).filter(Boolean)
    .map((line,i)=>{try{return JSON.parse(line) as JsonRecord;}catch(e){throw new Error(`Malformed JSONL at ${p}:${i+1}: ${(e as Error).message}`);}});
}
...
export function getTrainingAcademyCorpus(){
  ...
  const sourceIds=new Set(sources.map(s=>String(s.source_id)));
  for(const record of records){
...
export function getTrainingAcademyCounts(){
  ...
  return{learnerRecords:corpus.records.length,typeBreakdown,trackTaxonomy:corpus.tracks.length,populatedTracks:populatedTracks.size,learningPaths:corpus.learningPaths.length,sources:corpus.sources.length,relationships:corpus.relationships.length};
}
export function getTrainingRecordById(id:string){return getTrainingAcademyCorpus().records.find(r=>r.id===id);}
export function getTrainingRecordsByTrack(trackId:string){return getTrainingAcademyCorpus().records.filter(r=>r.track_id===trackId);}
export function getTrainingRecordsByType(recordType:string){return getTrainingAcademyCorpus().records.filter(r=>r.record_type===recordType);}
export function getTrainingSourceById(sourceId:string){return getTrainingAcademyCorpus().sources.find(s=>s.source_id===sourceId);}
export function getTrainingRelationshipsForRecord(id:string){return getTrainingAcademyCorpus().relationships.filter(e=>e.from_id===id||e.to_id===id);}
